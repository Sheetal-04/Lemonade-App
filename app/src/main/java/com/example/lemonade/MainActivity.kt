...
class MainActivity : AppCompatActivity() {
     ...
     */
    // Method will be called when user click on the image
    private fun clickLemonImage() {
        ...
        when(lemonadeState){
            ...
            SQUEEZE->{
                ...
                squeezeCount += 1
                lemonSize -= 1
                //now the lemonade will be in Squeeze state until we do not complete the whole lemon(lemonsize=0)
                //So if the lemonSize is 0,then the state will be DRINK till then we have to sqeeze
            ...
            // restart means now we will call have to select the lemon from the tree
            RESTART-> lemonadeState=SELECT
        }
        // TODO: lastly, before the function terminates we need to set the view elements so that the
        //  UI can reflect the correct state
        // As per result of the above we will call the setview elements() function and set the view accordingly
        setViewElements()
    }
    ...
    private fun setViewElements() {
        ...
        //Whatever the state is ,we have to set properties of image and text as follow
        when(lemonadeState){
            SELECT ->{
